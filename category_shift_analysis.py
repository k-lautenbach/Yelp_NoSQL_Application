...
# Incoming businesses associated with neighborhood upgrading
GENTRIFY_CATS = [
    # Food & drink — upscale/trendy
    "Coffee & Tea",
    "Cocktail Bars",
    "Wine Bars",
    "Wine & Spirits",
...
# ── Legacy / pre-gentrification categories ─────────────────
# Businesses associated with working-class neighborhoods
# or that tend to be displaced by gentrification
LEGACY_CATS = [
    # Fast / cheap food
    "Fast Food",
    "Burgers",
    ...
    "Body Shops",
    "Transmission Repair",
    # Ethnic / immigrant food (often displaced)
    "Soul Food",
    "Southern",
    "Barbeque",
    "Mexican",
    ...
    "Chinese",
    "Vietnamese",
    # Other working-class indicators
...
def build_category_timeline(neighborhood):
    """Returns dict: year → {category: count}"""
...
def get_gentrify_scores(timeline):
    """For each year, sum counts of gentrification vs legacy categories."""
...
# ── Step 4: Deep dive chart for Fountain Square ────────────

print(f"Analyzing {FOCUS_NEIGHBORHOOD}...")
fs_timeline = build_category_timeline(FOCUS_NEIGHBORHOOD)

# Get top 10 most common categories overall
all_cats = defaultdict(int)
for year_data in fs_timeline.values():
    for cat, count in year_data.items():
        all_cats[cat] += count

top_cats = [cat for cat, _ in sorted(all_cats.items(), key=lambda x: -x[1])[:10]]

# Build cumulative year-over-year for top categories
years = sorted(fs_timeline.keys())
df_data = {"year": years}
for cat in top_cats:
    df_data[cat] = [fs_timeline[y].get(cat, 0) for y in years]

df = pd.DataFrame(df_data).set_index("year")

# Smooth with 3-year rolling avg
...
for i, cat in enumerate(top_cats):
    ax.plot(df_smooth.index, df_smooth[cat],
...
ax.set_title(f"Business Category Composition Over Time — {FOCUS_NEIGHBORHOOD}", fontsize=13)
...
# ── Step 5: Gentrify vs Legacy score comparison chart ──────
...
for i, neighborhood in enumerate(all_neighborhoods):
    ...
    ax.plot(df_g_smooth.index, df_g_smooth["gentrify"],
            color="steelblue", linewidth=2, marker="o", label="Gentrification categories")
    ax.plot(df_g_smooth.index, df_g_smooth["legacy"],
            color="darkorange", linewidth=2, marker="o", label="Legacy categories")
